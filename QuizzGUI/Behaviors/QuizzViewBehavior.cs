...
{
    public class QuizzViewBehavior: Behavior<CarouselView>
    {
        ...
        private void Animation(CarouselView carousel, int selectedIndex)
        {
            if (carousel != null && carousel.ItemsSource != null && carousel.ItemsSource.Cast<object>().Count() > 0)
            {
                ...
                var ChildElement = (((CurrentItem as Quizz).CarouselItem as ContentView).Children[0] as StackLayout).Children.ToList();

                if (ChildElement != null && ChildElement.Count > 0)
                {
                    StartAnimation(ChildElement, CurrentItem as Quizz);
                }
                ...
                if (index != previousIndex)
                {
                    ...
                    var PreviousChildElement = (((PreviousItem as Quizz).CarouselItem as ContentView).Children[0] as StackLayout).Children.ToList();
        ...
        public async void StartAnimation(List<View> childElement, Quizz boarding)
        {
